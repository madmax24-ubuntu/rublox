...
class MockAudioContext {
  ...
  createGain() {
    const gain = { gain: { value: 1, setValueAtTime: () => {}, linearRampToValueAtTime: () => {}, exponentialRampToValueAtTime: () => {} } };
    gain.connect = () => gain;
    return gain;
  }
  createBiquadFilter() {
    const filter = { type: "lowpass", frequency: { value: 8000, setValueAtTime: () => {}, exponentialRampToValueAtTime: () => {} }, Q: { value: 0.8 } };
    filter.connect = () => filter;
    return filter;
  }
  createOscillator() {
    const osc = { type: "sine", frequency: { value: 440, setValueAtTime: () => {}, exponentialRampToValueAtTime: () => {} } };
    osc.connect = () => osc;
    osc.start = () => {};
    osc.stop = () => {};
    return osc;
  ...
  createDynamicsCompressor() {
    const comp = { threshold: { value: 0 }, knee: { value: 0 }, ratio: { value: 1 }, attack: { value: 0 }, release: { value: 0 } };
    comp.connect = () => comp;
    return comp;
  }
...
assert(audioSynth.bazookaLaunchBuffer !== null, "bazookaLaunchBuffer pre-generated");
assert(audioSynth.bazookaExplosionBuffer !== null, "bazookaExplosionBuffer pre-generated");
assert(audioSynth.bazookaHissBuffer !== null, "bazookaHissBuffer pre-generated");
assert(audioSynth.explosionCrackleBuffer !== null, "explosionCrackleBuffer pre-generated");
...
assert(typeof audioSynth.playBazooka === "function", "playBazooka is a function");
...
assert(playError === null, "playBazooka did not crash (was: pan.position is undefined)");
...
const hangarBuilding = mapGen._buildings.find((b) => b.template?.type === "hangar");
...
const hangarColliders = mapGen.colliders.filter((c) => {
  ...
  if (c.isBuildingWall) {
    ...
    return cx >= hx - hw / 2 - 3 && cx <= hx + hw / 2 + 3 && cz >= hz - hd / 2 - 3 && cz <= hz + hd / 2 + 3;
  }
  if (c.walkable) {
    const cx = (c.min.x + c.max.x) / 2;
    const cz = (c.min.z + c.max.z) / 2;
...
assert(hangarColliders.length > 0, `Found ${hangarColliders.length} hangar colliders`);
...
assert(wallColliders.every((c) => !c.walkable), "Wall colliders are non-walkable");
...
assert(floorColliders.length > 0, `Walkable floor colliders found: ${floorColliders.length}`);
...
assert(mg.colliderVersion > initialVersion, "colliderVersion incremented after addColliderBox");
...
assert(mg2.colliderVersion !== physics._colliderVersion, "Version mismatch detected");
...
assert(physics.colliderGridCount > initialGridCount, `Grid rebuilt: ${initialGridCount} -> ${physics.colliderGridCount}`);
