...
function mockParam(value = 0) {
    return {
        value,
        setValueAtTime: function (...args) { this.value = args[0]; },
        linearRampToValueAtTime: function (...args) { this.value = args[0]; },
        exponentialRampToValueAtTime: function (...args) { this.value = args[0]; },
...
class MockAudioContext {
    constructor() {
        ...
        this.listener = {
            positionX: mockParam(0), positionY: mockParam(0), positionZ: mockParam(0),
            forwardX: mockParam(1), forwardY: mockParam(0), forwardZ: mockParam(0),
            upX: mockParam(0), upY: mockParam(1), upZ: mockParam(0),
        };
    }
    async resume() {
        this.state = "running";
    ...
    createGain() {
        const gain = { gain: mockParam(1) };
        gain.connect = () => gain;
        return gain;
    }
    createBiquadFilter() {
        const filter = { type: "lowpass", frequency: mockParam(8000), Q: mockParam(0.8) };
        filter.connect = () => filter;
        return filter;
    }
    createOscillator() {
        const osc = { type: "sine", frequency: mockParam(440) };
        osc.connect = () => osc;
        osc.start = () => {};
        osc.stop = () => {};
        return osc;
    ...
    createPanner() {
        const panner = {
            ...
            maxDistance: 100,
            rolloffFactor: 1,
            positionX: mockParam(0),
            positionY: mockParam(0),
            positionZ: mockParam(0),
        };
        panner.connect = () => panner;
        return panner;
    }
    createConvolver() {
        const cvlt = { buffer: null };
        cvlt.connect = () => cvlt;
        return cvlt;
    }
