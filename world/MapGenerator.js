...
export class MapGenerator {
    ...
    _clearBiomeEntranceCorridors() {
        ...
        for (const child of [...this.scene.children]) {
            ...
            if (child.userData?.isBiomeEntrance || child.userData?.biomeBoundary || child.userData?.isCornucopia || child.userData?.isTowerStructure) continue;
        ...
        this.colliders = this.colliders.filter(collider => {
            if (!collider?.min || !collider?.max || collider.isBiomeEntrance || collider.biomeBoundary || collider.isCornucopia || collider.isTowerStructure) return true;
            const x = (collider.min.x + collider.max.x) * 0.5;
            const z = (collider.min.z + collider.max.z) * 0.5;
    ...
    _addLogCabin(x, z) {
        ...
        this.addColliderBox(new THREE.Vector3(x - w / 2, h / 2, z), wallThick, h, d, false);
        this.addColliderBox(new THREE.Vector3(x + w / 2, h / 2, z), wallThick, h, d, false);
        this.addColliderBox(new THREE.Vector3(x, h / 2, z - d / 2), w, h, wallThick, false);
        if (frontSegmentW > 0) {
            this.addColliderBox(new THREE.Vector3(x - (doorW + frontSegmentW) / 2, h / 2, z + d / 2), frontSegmentW, h, wallThick, false);
            this.addColliderBox(new THREE.Vector3(x + (doorW + frontSegmentW) / 2, h / 2, z + d / 2), frontSegmentW, h, wallThick, false);
        }
        this.addColliderBox(new THREE.Vector3(x, doorH + (h - doorH - 0.5) / 2, z + d / 2), w, h - doorH - 0.5, wallThick, false);
        this._buildings.push({ x, z, w, d, template: { type: 'log_cabin' } });
    }
    ...
    _generateMazeQuadrant() {
        ...
        while (stack.length) {
            ...
            for (const [dr, dc, side, opposite] of directions) {
                const nr = r + dr;
                const nc = c + dc;
                if (nr >= 0 && nr < mazeRows && nc >= 0 && nc < mazeCols && !cells[nr][nc].visited) {
            ...
            const [nr, nc, side, opposite] = candidates[Math.floor(this._rand() * candidates.length)];
        ...
        const mazeWalls = new THREE.InstancedMesh(wallGeometry, wallMat, segments.length);
        ...
        for (let i = 0; i < segments.length; i++) {
            ...
            this.addColliderBox(new THREE.Vector3(segment.x, wallHeight / 2, segment.z), segment.width, wallHeight, segment.depth, false);
        }
        ...
        for (let i = 0; i < towerWallSegments; i++) {
            ...
            const wallCollider = this.addColliderBox(new THREE.Vector3(sx, segmentY, sz), 0.8 * cosA + segmentLength * sinA, segmentHeight, 0.8 * sinA + segmentLength * cosA, false);
    ...
    _addMilitaryHangar(x, z, w, d, h) {
        ...
        if (doorLeftW > 0) {
            const dl = new THREE.Mesh(this.pool.getGeoBox(doorLeftW, h, 0.9), wallMat);
            dl.position.set(-doorLeftW / 2 - 0.5, h / 2, d / 2);
            dl.userData.mapGenerated = true;
            dl.userData.isWall = true;
            group.add(dl);
            this.addColliderBox(new THREE.Vector3(x - doorLeftW / 2 - 0.5, h / 2, z + d / 2), doorLeftW, h, 0.9, false);
        }
        if (doorRightW > 0) {
            const dr = new THREE.Mesh(this.pool.getGeoBox(doorRightW, h, 0.9), wallMat);
            dr.position.set(doorRightW / 2 + 0.5, h / 2, d / 2);
            dr.userData.mapGenerated = true;
            dr.userData.isWall = true;
            group.add(dr);
            this.addColliderBox(new THREE.Vector3(x + doorRightW / 2 + 0.5, h / 2, z + d / 2), doorRightW, h, 0.9, false);
        }
        if (doorTopH > 0) {
            ...
            group.add(dt);
            this.addColliderBox(new THREE.Vector3(x, doorH + doorTopH / 2, z + d / 2), w, doorTopH, 0.9, false);
        }
        const door = new THREE.Mesh(this.pool.getGeoBox(doorW, doorH, 0.1), doorMat);
        door.position.set(0, doorH / 2, d / 2 + 0.05);
        door.userData.mapGenerated = true;
        ...
        backWall.userData.isWall = true;
        group.add(backWall);
        this.addColliderBox(new THREE.Vector3(x, h / 2, z - d / 2), w, h, 0.9, false);
        if (doorLeftW > 0) {
            this.addColliderBox(new THREE.Vector3(x - doorLeftW / 2 - 0.5, h / 2, z + d / 2), doorLeftW, h, 0.9, false);
            this.addColliderBox(new THREE.Vector3(x + doorRightW / 2 + 0.5, h / 2, z + d / 2), doorRightW, h, 0.9, false);
        }
