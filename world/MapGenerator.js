        ...
        for (const sector of this.voronoi.sectors) {
            ...
            if (sector.biome === 'stone_maze') {
                ...
                for (let t = 0; t < numTanks; t++) {
                    ...
                    const tx = cx + Math.cos(tankAngle) * tankDist;
                    const tz = cz + Math.sin(tankAngle) * tankDist;
            ...
            } else if (sector.biome === 'ice_lake') {
                // Ice lake: add crystal formations around the sector
            ...
            } else if (sector.biome === 'ruins') {
                // Ruins: add rubble and debris piles instead of standard props
                for (let i = 0; i < numProps * 2; i++) {
                    const angle = this._rand() * Math.PI * 2;
                    const dist = 8 + this._rand() * radius * 0.6;
                    const px = cx + Math.cos(angle) * dist;
                    const pz = cz + Math.sin(angle) * dist;
                    const baseY = this.getHeightAt(px, pz);

                    // Rubble pile: cluster of small irregular stones
                    for (let j = 0; j < 3 + Math.floor(this._rand() * 4); j++) {
                        ...
                        rubble.position.set(px + (this._rand() - 0.5) * sSize * 2, baseY + sSize / 2, pz + (this._rand() - 0.5) * sSize * 2);
                    ...
                    if (this._rand() < 0.35) {
                        ...
                        wallFrag.position.set(px + (this._rand() - 0.5) * 4, baseY + wH / 2, pz + (this._rand() - 0.5) * 4);
                        wallFrag.rotation.z = (this._rand() - 0.5) * 0.15; // Slight lean for ruined look
                        wallFrag.userData.mapGenerated = true;
                        this.scene.add(wallFrag);

                        const wColliderGeo = new THREE.BoxGeometry(2, wH + 0.3, 0.6);
                    }
                }
            } else if (sector.biome === 'swamp') {
                // Swamp: add dead trees and murky water surface props instead of normal grass/trees
                for (let i = 0; i < Math.floor(numTrees * 0.4); i++) {
                    const angle = this._rand() * Math.PI * 2;
                    const dist = 15 + this._rand() * radius * 0.7;
                    const sx = cx + Math.cos(angle) * dist;
                    const sz = cz + Math.sin(angle) * dist;

                    // Dead tree (bare trunk, no crown - dark brown/grey color)
                    const deadTrunkGeo = new THREE.CylinderGeometry(0.2, 0.5, 4 + this._rand() * 3, 6);
                    const deadTreeMat = new THREE.MeshStandardMaterial({ color: 0x3e2723, roughness: 1 });

                    // Branches (sparse angular protrusions from trunk)
                    for (let b = 0; b < Math.floor(2 + this._rand() * 4); b++) {
                        const branchGeo = new THREE.CylinderGeometry(0.05, 0.15, 1.5 + this._rand(), 3);
                        const branch = new THREE.Mesh(branchGeo, deadTreeMat.clone());

                    // Root system (exposed roots at base spreading into the mud)
                    for (let rIdx = 0; rIdx < 4; rIdx++) {
                        const rootAngle = this._rand() * Math.PI * 2;
                        const rootGeo = new THREE.CylinderGeometry(0.03, 0.15, 1 + this._rand(), 3);
                    }

                // --- Add collider boxes for all tank parts (non-walkable obstacle zone) ---
            } else if (sector.biome === 'industrial') {
                // Industrial: add storage tanks and pipes instead of standard props
                const numTanks = Math.floor(4 + sector.buildingDensity * 4);
                for (let i = 0; i < numTanks; i++) {
                    const angle = this._rand() * Math.PI * 2;
                    const dist = radius * 0.3 + this._rand() * radius * 0.6;

                    // Storage tank body (large vertical cylinder with hemispherical top)
                    const tGeo = new THREE.CylinderGeometry(1.5, 1.5, 4, 8);
                    const tTopGeo = new THREE.SphereGeometry(1.5, 8, 6, 0, Math.PI / 2);

                    // Pipe segments connecting tanks in industrial zone (horizontal and vertical runs)
                        if (!this.animatedObjects) this.animatedObjects = [];
                }
            } else {
                // Default: add ice crystal formations around the sector bounds using golden angle distribution for even spacing. The radius is calculated from half the total size, then the code loops through sectors to find matching biome types and places crystals at regular intervals along each edge with a slight random offset for natural variation. Finally it logs how many crystals were added per sector.
            }
        }
    }
