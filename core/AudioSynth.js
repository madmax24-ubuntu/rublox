...
export class AudioSynth {
	constructor() {
		...
		this.sampleCatalog = {
			...
			footsteps: Array.from(
				{ length: 10 },
				(_, i) => `assets/audio/rpg/footstep0${i}.ogg`,
			),
			hit: [
			...
			hurt: ["assets/audio/rpg/cloth3.ogg", "assets/audio/rpg/cloth4.ogg"],
			playerHurt: [
			...
			zombieMoan: Array.from(
				{ length: 12 },
				(_, i) => `assets/audio/zombies/zombie-${i + 1}.wav`,
			),
			zombieAttack: Array.from(
				{ length: 12 },
				(_, i) => `assets/audio/zombies/zombie-${i + 13}.wav`,
			),
			bow: ["assets/audio/weapons/bow_shot.wav"],
			laser: ["assets/audio/weapons/laser_shot.wav"],
			machinegun: [
				"assets/audio/weapons/machinegun_ppsh_a.wav",
				"assets/audio/weapons/machinegun_ppsh_b.wav",
			],
			shotgun: ["assets/audio/weapons/shotgun_model12_a.wav"],
			pistol: ["assets/audio/weapons/pistol_1911_a.wav"],
			rifle: ["assets/audio/weapons/rifle_ar15_a.wav"],
			flamethrower: ["assets/audio/weapons/flamethrower_fire.ogg"],
			reload: [
	...
	async init() {
		try {
			...
			if (this._useWorker) {
				const [launchData, explosionData, hissData, crackleData, impulseData] =
					await Promise.all([
						this._postWorker("bazookaLaunch", { rate, dur: 0.9 }),
						this._postWorker("bazookaExplosion", { rate, dur: 2.5 }),
						this._postWorker("bazookaHiss", { rate, dur: 0.9 }),
						this._postWorker("explosionCrackle", { rate, dur: 0.6 }),
				...
				this.bazookaHissBuffer = this._createBufferFromData(1, hissData, rate);
				this.explosionCrackleBuffer = this._createBufferFromData(1, crackleData, rate);
	...
	_createSyncBuffers(rate) {
		...
		this.bazookaHissBuffer = mkBuf(0.9, (t) => {
			const e = t < 0.05 ? t / 0.05 : t < 0.5 ? 1 : Math.max(0, 1 - (t - 0.5) / 0.4);
			return (Math.random() * 2 - 1) * e * 0.35;
		});
		this.explosionCrackleBuffer = mkBuf(0.6, (t) => {
			return (Math.random() * 2 - 1) * Math.exp(-t * 4) * 0.45;
		});
	...
	getCategoryGain(category) {
		const key = String(category || "").toLowerCase();
		return (
			this.categoryGains[key] || this.categoryGains.sfx || this.masterSfxGain
		);
	}
	...
	async playSample(pathList, options = {}) {
		...
		if (source.loop) {
			source.loopStart = clamp(
				Number.isFinite(options.loopStart) ? options.loopStart : 0,
				0,
				Math.max(0, buffer.duration - 0.02),
			);
			source.loopEnd = clamp(
				Number.isFinite(options.loopEnd) ? options.loopEnd : buffer.duration,
		...
		const rate = Number.isFinite(options.rate)
			? options.rate
			: (Number.isFinite(options.rateMin) ? options.rateMin : 1) +
				Math.random() *
					((Number.isFinite(options.rateMax) ? options.rateMax : 1) -
						(Number.isFinite(options.rateMin) ? options.rateMin : 1));
		source.playbackRate.value = clamp(rate, 0.5, 2.25);

		const gainNode = ctx.createGain();
		gainNode.gain.value = clamp(
			Number.isFinite(options.volume) ? options.volume : 0.2,
			0,
			2,
		);
		...
		if (voiceKey) {
			...
			if (previous) {
				...
				try {
					...
					previous.gain.gain.exponentialRampToValueAtTime(
						0.0001,
						fadeAt + 0.018,
					);
			...
			source.addEventListener(
				"ended",
				() => {
					if (this.activeSampleVoices.get(voiceKey)?.source === source)
	...
	async playProceduralShot(
		...
		f.type = "lowpass";
		f.frequency.value =
			kind === "shotgun" ? 1200 : kind === "flamethrower" ? 2200 : 2600;
	...
	canPlayWeaponSfx(type, minInterval = 0) {
		...
		const interval = Math.max(
			0,
			Number.isFinite(minInterval) ? minInterval : 0,
		);
	...
	getEmitterSfxScale(emitterKey) {
		return String(emitterKey || "").startsWith("id:")
			? this.isMobileDevice
				? 0.16
				: 0.21
			: 1;
	...
	setBiomeAmbience(biome) {
		...
		gain.gain.linearRampToValueAtTime(
			profile.gain * (this.isMobileDevice ? 0.8 : 1),
			now + 0.8,
		);
	...
	playGrieverRoar(position) {
		this.playSample(this.sampleCatalog.zombieAttack, {
			volume: 0.2,
			rateMin: 0.5,
			rateMax: 0.8,
			position,
	...
	playGrieverAttack(position) {
		this.playSample(this.sampleCatalog.hit, {
			volume: 0.2,
			rateMin: 0.6,
			rateMax: 0.95,
			position,
	...
	playFootstep(volume = 1) {
		const gainScale =
			clamp(volume, 0.15, 1.2) * (this.footstepWeatherFactor || 1);
	...
	playHit(position = null, emitterKey = "global") {
		...
		this.playSample(this.sampleCatalog.hit, {
			volume:
				(this.isMobileDevice ? 0.26 : 0.34) *
				this.getEmitterSfxScale(emitterKey),
			rateMin: 0.95,
			rateMax: 1.12,
			reverbSend: 0.06,
			position,
	...
	stopRadiationRain() {
		...
		if (this.radiationRainNodes.radiationTimer)
			clearInterval(this.radiationRainNodes.radiationTimer);
		this.radiationRainNodes = null;
	}

	playHurt(position = null, emitterKey = "global") {
		if (
			!this.canPlayWeaponSfx(
				`hurt:${emitterKey}`,
				emitterKey === "global" ? 0.08 : 0.2,
			)
		)
			return;
	...
	playZombieAttack(position = null, opts = null) {
		...
		this.playSample(this.sampleCatalog.zombieAttack.slice(offset, offset + 2), {
			volume: this.isMobileDevice ? 0.16 : 0.24,
			rateMin: rates[0],
			rateMax: rates[1],
			position,
			reverbSend: 0.08,
			category: "zombie",
			priority: 0,
			maxDuration: variant === "heavy" ? 0.65 : 0.42,
			voiceKey: `zombie:attack:${emitterKey}`,
		});
	}
	...
	playZombieAbility(position = null, opts = null) {
		...
		const [offset, rateMin, rateMax, duration] =
			profiles[variant] || profiles.heavy;
		this.playSample(this.sampleCatalog.zombieAttack.slice(offset, offset + 2), {
	...
	playZoneDamage() {
		this.playSample(this.sampleCatalog.zoneDamage, {
			volume: this.isMobileDevice ? 0.08 : 0.11,
			rateMin: 0.6,
			rateMax: 1.4,
			category: "weather",
			maxDuration: 0.3,
	...
	playZombieMoan(position = null, opts = null) {
		...
		const [offset, rateMin, rateMax] = profiles[variant] || profiles.normal;
		const rates = [rateMin, rateMax];
		this.playSample(this.sampleCatalog.zombieMoan.slice(offset, offset + 2), {
			...
			category: "zombie",
			priority: 0,
			maxDuration: variant === "heavy" ? 1.1 : 0.75,
			voiceKey: `zombie:moan:${emitterKey}`,
		});
	}
		...
		const globalInterval =
			kind === "attack"
				...
				: this.isMobileDevice
					? 0.55
					: 0.4;
		if (now - (this.lastZombieSfxTime[kind] || 0) < globalInterval)
			return false;
		const key = `${kind}:${emitterKey}`;
		if (now - (this.lastZombieEmitterSfx.get(key) || 0) < emitterInterval)
			return false;
	...
	playReload() {
		this.playSample(this.sampleCatalog.reload, {
			volume: this.isMobileDevice ? 0.15 : 0.22,
			rateMin: 0.9,
			rateMax: 1.15,
			category: "sfx",
		});
	...
	playPickup() {
		this.playSample(this.sampleCatalog.pickup, {
	...
	playDeath(position = null) {
		this.playSample(this.sampleCatalog.death, {
			volume: this.isMobileDevice ? 0.3 : 0.42,
			rateMin: 0.7,
			rateMax: 0.95,
			position,
			category: "sfx",
	...
	playUISound() {
		this.playSample(this.sampleCatalog.ui, {
			volume: this.isMobileDevice ? 0.1 : 0.15,
			rateMin: 1.0,
			rateMax: 1.0,
			category: "ui",
		});
	...
	playBowShot(position = null, emitterKey = "global") {
		if (!this.canPlayWeaponSfx(`bow:${emitterKey}`, this.weaponSfxCooldown.bow))
			return;
		this.playSample(this.sampleCatalog.bow, {
			volume:
				(this.isMobileDevice ? 0.72 : 0.82) *
				this.getEmitterSfxScale(emitterKey),
			rateMin: 0.99,
			rateMax: 1.01,
			position,
			...
			if (!played)
				this.playProceduralShot(
					"bow",
					0.2 * this.getEmitterSfxScale(emitterKey),
					position,
				);
		});
	}
	...
	playLaser(position = null, emitterKey = "global") {
		if (
			!this.canPlayWeaponSfx(
				`laser:${emitterKey}`,
				this.weaponSfxCooldown.laser,
			)
		)
			return;
		const scale = this.getEmitterSfxScale(emitterKey);
		this.playSample(this.sampleCatalog.laser, {
			volume: (this.isMobileDevice ? 0.72 : 0.82) * scale,
			rateMin: 0.99,
			rateMax: 1.01,
			position,
			category: "weapon",
			...
			if (!played) this.playProceduralShot("laser", 0.2 * scale, position);
		});
	}
	...
	playShotgun(volume = 1, position = null, emitterKey = "global") {
		if (
			!this.canPlayWeaponSfx(
				`shotgun:${emitterKey}`,
				this.weaponSfxCooldown.shotgun,
			)
		)
			return;
		...
		this.playSample(this.sampleCatalog.shotgun, {
			...
			if (!played)
				this.playProceduralShot("shotgun", 0.32 * scaled * scale, position);
		});
	}
	...
	playMachinegun(position = null, emitterKey = "global") {
		if (
			!this.canPlayWeaponSfx(
				`machinegun:${emitterKey}`,
				this.weaponSfxCooldown.machinegun,
			)
		)
			return;
		const scale = this.getEmitterSfxScale(emitterKey);
	...
	playFlamethrower(position = null, emitterKey = "global") {
		...
		const refreshStop = () => {
			...
			this.flamethrowerStopTimers.set(
				voiceKey,
				setTimeout(() => {
		...
		if (
			this.activeSampleVoices.has(voiceKey) ||
			this.flamethrowerPending.has(voiceKey)
		) {
			refreshStop();
			return true;
		}
		if (!this.canPlayWeaponSfx(voiceKey, this.weaponSfxCooldown.flamethrower))
			return false;
		const scale = this.getEmitterSfxScale(emitterKey);
	...
	setWeatherState(nextState = "clear") {
		...
		this.weatherTransitionTimer = null;
		this.stopWeatherLoop();
		this.footstepWeatherFactor =
			state === "rain" ? 0.62 : state === "snow" ? 0.82 : 1;
	...
	playChestOpen(position = null) {
		this.playSample(this.sampleCatalog.chestOpen, {
			volume: this.isMobileDevice ? 0.1 : 0.16,
			rateMin: 0.92,
			rateMax: 1.08,
			position,
			reverbSend: 0.12,
	...
	createSurvivalMusicBuffer() {
		...
		for (let channel = 0; channel < 2; channel++) {
			...
			for (let i = 0; i < data.length; i++) {
				...
				const breeze =
					(air - drift) * (0.034 + 0.009 * Math.sin(t * 0.11 + channel * 0.3));
				data[i] = breeze * fade;
			}
		}
	...
	playMusic() {
		...
		const playMusicTheme = (index) => {
			...
			const theme =
				this.sampleCatalog.music[index % this.sampleCatalog.music.length];
			const buffer = theme
				? this.sampleBuffers.get(theme)
				: this.createSurvivalMusicBuffer();
			if (!buffer) return;
	...
	applyMusicWeatherProfile(state = "clear") {
		...
		this.musicGain.gain.linearRampToValueAtTime(
			this.musicVolume * profile.gain,
			now + 2.5,
		);
		if (this.musicSource?.playbackRate) {
			this.musicSource.playbackRate.cancelScheduledValues(now);
			this.musicSource.playbackRate.setValueAtTime(
				this.musicSource.playbackRate.value,
				now,
			);
