...
export class AudioSynth {
	constructor() {
		...
		// Pre-generated procedural buffers for bazooka (generated by worker — no sync freeze)
		this.bazookaLaunchBuffer = null;
		this.bazookaExplosionBuffer = null;
		this.bazookaLaunchRealBuffer = null;
		this.bazookaExplosionRealBuffer = null;
		// Fallback audio files (decoded async — no freeze)
		this._bazookaLaunchFallbackPath = "assets/audio/weapons/bazooka_launch_real.mp3";
		this._bazookaExplosionFallbackPath = "assets/audio/weapons/bazooka_explosion_real.mp3?v=3";
	...
	// Async fallback: decode existing audio files (no freeze)
	// Called before playBazooka / playProceduralExplosion when worker buffer not ready
	async _ensureBazookaBuffersReady() {
		...
		this._bazookaFallbackLoading = (async () => {
			try {
				const [launchRes, explosionRes] = await Promise.all([
					fetch(this._bazookaLaunchFallbackPath),
					fetch(this._bazookaExplosionFallbackPath),
				]);
				const [launchBuf, explosionBuf] = await Promise.all([
				...
				if (!this.bazookaLaunchRealBuffer && !this.bazookaLaunchBuffer) this.bazookaLaunchBuffer = this._makeSimpleLaunchBuffer();
				if (!this.bazookaExplosionRealBuffer && !this.bazookaExplosionBuffer) this.bazookaExplosionBuffer = this._makeSimpleExplosionBuffer();
			}
			this.setupBazookaFallbackInSampleBuffers();
	...
	setupBazookaFallbackInSampleBuffers() {
		const launch = this.bazookaLaunchRealBuffer ?? this.bazookaLaunchBuffer;
		const explosion = this.bazookaExplosionRealBuffer ?? this.bazookaExplosionBuffer;
		if (launch) this.sampleBuffers.set(this._bazookaLaunchFallbackPath, launch);
		if (explosion) this.sampleBuffers.set(this._bazookaExplosionFallbackPath, explosion);
	}
	...
	_makeSimpleLaunchBuffer() {
		const ctx = this.audioContext;
		const rate = ctx.sampleRate;
		const len = Math.floor(rate * 0.5);
		const buf = ctx.createBuffer(1, len, rate);
		const d = buf.getChannelData(0);
		for (let i = 0; i < len; i++) {
			const t = i / rate;
			const env = t < 0.02 ? t / 0.02 : Math.exp(-t * 5);
			d[i] = (Math.random() * 2 - 1) * env * 0.8 + Math.exp(-t * 100) * 1.2;
		}
		return buf;
	}

	// Simple procedural explosion buffer (fast to generate, used only as last-resort fallback)
	...
	async loadSamples() {
		...
		this.sampleLoadPromise = Promise.all(
			paths.map(async (path) => {
				...
				if (path === this._bazookaLaunchFallbackPath)
					this.bazookaLaunchRealBuffer = buffer;
				else if (path === this._bazookaExplosionFallbackPath)
					this.bazookaExplosionRealBuffer = buffer;
				} catch (_) {}
			}),
		);
