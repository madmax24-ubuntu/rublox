...
class Game {
    ...
    updatePlatformGateCycle(delta) {
        ...
        this.platformGateCycleTimer = 30;
        this.setCenterPlatformOpen(true);
        this.hud.showGameMessage('Ворота центральной платформы открыты на 30 секунд!');
    }
    ...
    _updateZombieWaves(delta) {
        ...
        if (this.zombieMaintainTimer <= 0) {
            ...
            const aliveZombies = this._zombieAliveCount ?? this.zombies.filter(z => z?.isAlive).length;
