...
import {
	createPBRMetalTexture,
	createPolymerTexture2,
	createWoodTexture2,
	createBrightSteelTexture,
	createWeaponMaterial,
} from "./WeaponTextures.js";
...
function createWoodTexture(baseColor = "#5a3a20") {
	return createCanvasTexture(512, 512, (ctx, w, h) => {
		...
		for (let row = 0; row < 40; row++) {
			const baseY = (row / 40) * h,
				d = 0.08 + Math.random() * 0.18;
			...
			for (let x = 0; x <= w; x += 8) {
				const w =
					Math.sin(x * 0.01 + row) * 8 + Math.sin(x * 0.03 + row * 0.5) * 4;
		...
		for (let i = 0; i < 5; i++) {
			...
			for (let a = 0; a < Math.PI * 2; a += 0.1) {
				const r = 20 + a * 8 + Math.sin(a * 3) * 5;
				ctx.lineTo(cx + Math.cos(a) * r * 0.5, cy + r);
			}
			ctx.stroke();
		}
	});
...
export class WeaponAnimation {
	...
	update(delta, isShooting, isMoving, mouseDx, mouseDy) {
		...
		this.recoilAngle +=
			(this.recoilKick * 0.115 - this.recoilAngle) * Math.min(1, delta * 24);
		if (this.muzzleFlash > 0)
			this.muzzleFlash = Math.max(0, this.muzzleFlash - delta * 14);
	...
	triggerRecoil(amount) {
		...
		this.heatGlow = Math.min(1, this.heatGlow + 0.2);
	}
	triggerReload(dur) {
...
// ═══════════════════════════════════════════════════════════════════════════════════════════════
export function createGunshotSound(
	type,
	audioContext,
	volume = 0.5,
	_position = null,
	...
	const mkNoise = (dur, gMul, hp, lp, bp) => {
		const buf = audioContext.createBuffer(
			1,
			Math.max(1, Math.floor(audioContext.sampleRate * dur)),
		...
		g.gain.exponentialRampToValueAtTime(Math.max(0.001, gMul), t + 0.003);
	...
	const mkOsc = (freq, toFreq, dur, gMul, type = "sine") => {
		...
		o.frequency.exponentialRampToValueAtTime(Math.max(25, toFreq), now + dur);
		const g = audioContext.createGain();
		g.gain.setValueAtTime(Math.max(0.001, gMul), now);
	...
	switch (type) {
		...
		case "laser": {
			...
			const o = audioContext.createOscillator();
			o.type = "sine";
			o.frequency.setValueAtTime(1800, now + 0.03);
...
function createKnifeModel() {
	...
	const bladeMat = getMaterial("knife_blade_pbr", () =>
		createWeaponMaterial(createBrightSteelTexture()),
	);
	const handleMat = getMaterial("knife_handle_pbr", () =>
		createWeaponMaterial(createWoodTexture2("#5a3a24")),
	);
	const guardMat = getMaterial("knife_guard_pbr", () =>
		createWeaponMaterial(createPBRMetalTexture("#909ca8")),
	);
	const pommelMat = getMaterial("knife_pommel_pbr", () =>
		createWeaponMaterial(createPolymerTexture2("#59636e")),
	);

	// Handle scales with rivets
	group.add(
		createPart(
			getGeom("knife_h1", () => new THREE.BoxGeometry(0.42, 0.11, 0.11)),
			handleMat,
			-0.27,
			0,
			0,
	...
	group.add(
		createPart(
			getGeom("knife_h2", () => new THREE.BoxGeometry(0.16, 0.13, 0.13)),
			handleMat,
			-0.42,
			0,
			0,
	...
	group.add(
		createPart(
			getGeom("knife_g", () => new THREE.BoxGeometry(0.08, 0.16, 0.16)),
			guardMat,
			-0.03,
			0,
			0,
	...
	group.add(
		createPart(
			getGeom("knife_gw", () => new THREE.BoxGeometry(0.04, 0.06, 0.18)),
			guardMat,
			-0.06,
			0.03,
			0,
	...
	group.add(
		createPart(
			getGeom("knife_b", () => new THREE.BoxGeometry(0.82, 0.055, 0.045)),
			bladeMat,
			0.4,
			0.01,
			0,
	...
	group.add(
		createPart(
			getGeom("knife_bs", () => new THREE.BoxGeometry(0.72, 0.02, 0.045)),
			guardMat,
			0.38,
			0.042,
			0,
	...
	group.add(
		createPart(
			getGeom("knife_f", () => new THREE.BoxGeometry(0.5, 0.005, 0.008)),
			guardMat,
			0.42,
			0.005,
			0,
	...
	group.add(
		createPart(
			getGeom("knife_t", () => new THREE.ConeGeometry(0.038, 0.18, 4)),
			bladeMat,
			0.86,
			0.01,
			0,
	...
	group.add(
		createPart(
			getGeom("knife_p", () => new THREE.ConeGeometry(0.07, 0.06, 6)),
			pommelMat,
			-0.52,
			0,
			0,
...
function createBowModel() {
	...
	const segGeom = getGeom(
		"bow_seg",
		() => new THREE.BoxGeometry(0.14, 0.35, 0.09),
	);
	const segData = [
		[-0.16, 0.88, 0.48, 0.08],
		[-0.08, 0.52, 0.26, 0.05],
	...
	group.add(
		createPart(
			getGeom("bow_tip_t", () => new THREE.ConeGeometry(0.06, 0.12, 4)),
			limbMat,
			-0.14,
			1.0,
			0,
	...
	group.add(
		createPart(
			getGeom("bow_tip_b", () => new THREE.ConeGeometry(0.06, 0.12, 4)),
			limbMat,
			-0.14,
			-1.0,
			0,
	...
	group.add(
		createPart(
			getGeom("bow_grip", () => new THREE.BoxGeometry(0.18, 0.64, 0.13)),
			gripMat,
			0.02,
			0,
			0,
	...
	group.add(
		createPart(
			getGeom("bow_grip_w", () => new THREE.BoxGeometry(0.19, 0.3, 0.14)),
			gripMat,
			0.02,
			0.05,
			0,
...
function createGunModel(style) {
	...
	if (style === "pistol") {
		steelMat = getMaterial("pistol_steel_pbr", () =>
			createWeaponMaterial(createPBRMetalTexture("#b6c2cc")),
		);
		darkMat = getMaterial("pistol_dark_pbr", () =>
			createWeaponMaterial(createPolymerTexture2("#515b66")),
		);
		gripMat = getMaterial("pistol_grip_pbr", () =>
			createWeaponMaterial(createPolymerTexture2("#626d78")),
		);
		woodMat = getMaterial("pistol_wood_pbr", () =>
			createWeaponMaterial(createWoodTexture2("#70482a")),
		);
		brassMat = getMaterial(
			"pistol_brass",
			() =>
				new THREE.MeshStandardMaterial({
					color: 0xb5a040,
		...
		steelMat = getMaterial(`${style}_steel_pbr`, () =>
			createWeaponMaterial(
				createPBRMetalTexture(style === "machinegun" ? "#74818d" : "#929faa"),
			),
		);
		darkMat = getMaterial(`${style}_dark_pbr`, () =>
			createWeaponMaterial(createPolymerTexture2("#515b66")),
		);
		gripMat = getMaterial(`${style}_grip_pbr`, () =>
			createWeaponMaterial(createPolymerTexture2("#626d78")),
		);
		woodMat = getMaterial(`${style}_wood_pbr`, () =>
			createWeaponMaterial(createWoodTexture2("#70482a")),
		);
		brassMat = getMaterial(
			`${style}_brass`,
		...
		steelMat = getMaterial(`${style}_steel_pbr`, () =>
			createWeaponMaterial(
				createPBRMetalTexture(style === "laser" ? "#5f91a6" : "#87939d"),
			),
		);
		darkMat = getMaterial(`${style}_dark_pbr`, () =>
			createWeaponMaterial(createPolymerTexture2("#515b66")),
	...
	if (style === "pistol") {
		...
		group.add(
			createPart(
				getGeom("pistol_frame", () => new THREE.BoxGeometry(0.5, 0.15, 0.13)),
				darkMat,
				0.04,
				-0.04,
				0,
		...
		group.add(
			createPart(
				getGeom("pistol_slide", () => new THREE.BoxGeometry(0.42, 0.13, 0.15)),
				steelMat,
				0.06,
				0.1,
				0,
		...
		for (let i = 0; i < 6; i++)
			group.add(
				createPart(
					getGeom(
						`pistol_serr${i}`,
						() => new THREE.BoxGeometry(0.015, 0.02, 0.16),
					),
					accentMat,
					-0.12 + i * 0.02,
					0.14,
		...
		group.add(
			createPart(
				getGeom("pistol_grip", () => new THREE.BoxGeometry(0.14, 0.28, 0.12)),
				darkMat,
				-0.08,
				-0.18,
				0,
		...
		group.add(
			createPart(
				getGeom("pistol_gp", () => new THREE.BoxGeometry(0.12, 0.24, 0.01)),
				gripMat,
				-0.08,
				-0.16,
				0.07,
		...
		group.add(
			createPart(
				getGeom("pistol_mag", () => new THREE.BoxGeometry(0.16, 0.05, 0.14)),
				brassMat,
				-0.08,
				-0.33,
				0,
		...
		group.add(
			createPart(
				getGeom("pistol_ss", () => new THREE.BoxGeometry(0.04, 0.03, 0.03)),
				darkMat,
				-0.08,
				0.06,
				-0.09,
		...
		group.add(
			createPart(
				getGeom("pistol_rs", () => new THREE.BoxGeometry(0.06, 0.06, 0.06)),
				darkMat,
				-0.18,
				0.17,
				0,
		...
		group.add(
			createPart(
				getGeom("pistol_rs_p", () => new THREE.BoxGeometry(0.02, 0.04, 0.04)),
				neonMat,
				-0.18,
				0.21,
				0,
		...
		group.add(
			createPart(
				getGeom(
					"rifle_receiver",
					() => new THREE.BoxGeometry(0.48, 0.18, 0.14),
				),
				darkMat,
				0.02,
				0.03,
		...
		group.add(
			createPart(
				getGeom(
					"rifle_dust_cover",
					() => new THREE.BoxGeometry(0.34, 0.08, 0.15),
				),
				steelMat,
				0.06,
				0.15,
		...
		group.add(
			createPart(
				getGeom(
					"rifle_wood_guard",
					() => new THREE.BoxGeometry(0.5, 0.17, 0.17),
				),
				woodMat,
				0.5,
				0.04,
		...
		group.add(
			createPart(
				getGeom(
					"rifle_long_barrel",
					() => new THREE.CylinderGeometry(0.035, 0.035, 0.86, 10),
				),
				steelMat,
				1.02,
				0.07,
		...
		group.add(
			createPart(
				getGeom(
					"rifle_muzzle",
					() => new THREE.CylinderGeometry(0.055, 0.045, 0.15, 8),
				),
				darkMat,
				1.48,
				0.07,
		...
		group.add(
			createPart(
				getGeom("rifle_stock", () => new THREE.BoxGeometry(0.58, 0.22, 0.17)),
				woodMat,
				-0.48,
				0.01,
				0,
		...
		group.add(
			createPart(
				getGeom(
					"rifle_stock_comb",
					() => new THREE.BoxGeometry(0.38, 0.07, 0.18),
				),
				woodMat,
				-0.38,
				0.16,
		...
		group.add(
			createPart(
				getGeom("rifle_butt", () => new THREE.BoxGeometry(0.06, 0.24, 0.18)),
				gripMat,
				-0.79,
				-0.01,
				0,
		...
		group.add(
			createPart(
				getGeom(
					"rifle_curved_mag",
					() => new THREE.BoxGeometry(0.14, 0.34, 0.1),
				),
				darkMat,
				0.08,
				-0.22,
		...
		group.add(
			createPart(
				getGeom("rifle_mag_lip", () => new THREE.BoxGeometry(0.16, 0.05, 0.11)),
				brassMat,
				0.12,
				-0.39,
				0,
		...
		group.add(
			createPart(
				getGeom("rifle_grip", () => new THREE.BoxGeometry(0.12, 0.25, 0.12)),
				gripMat,
				-0.14,
				-0.18,
				0,
		...
		const oliveMat = getMaterial("machinegun_olive", () =>
			createWeaponMaterial(createPolymerTexture2("#596348")),
		);
		group.add(
			createPart(
				getGeom("mg_receiver", () => new THREE.BoxGeometry(0.68, 0.28, 0.24)),
				darkMat,
				0.02,
				0.03,
				0,
		...
		group.add(
			createPart(
				getGeom("mg_top_cover", () => new THREE.BoxGeometry(0.58, 0.09, 0.26)),
				oliveMat,
				0.01,
				0.22,
				0,
		...
		group.add(
			createPart(
				getGeom(
					"mg_heavy_barrel",
					() => new THREE.CylinderGeometry(0.055, 0.055, 0.96, 10),
				),
				steelMat,
				0.84,
				0.08,
		...
		group.add(
			createPart(
				getGeom(
					"mg_barrel_jacket",
					() => new THREE.CylinderGeometry(0.09, 0.09, 0.56, 10),
				),
				darkMat,
				0.66,
				0.08,
		...
		for (let i = 0; i < 6; i++)
			group.add(
				createPart(
					getGeom(
						`mg_cooling_${i}`,
						() => new THREE.TorusGeometry(0.095, 0.012, 5, 10),
					),
					steelMat,
					0.44 + i * 0.09,
					0.08,
		...
		group.add(
			createPart(
				getGeom(
					"mg_flash_hider",
					() => new THREE.CylinderGeometry(0.08, 0.055, 0.18, 8),
				),
				darkMat,
				1.38,
				0.08,
		...
		group.add(
			createPart(
				getGeom("mg_box_mag", () => new THREE.BoxGeometry(0.34, 0.38, 0.22)),
				oliveMat,
				-0.02,
				-0.28,
				0,
		...
		group.add(
			createPart(
				getGeom("mg_feed", () => new THREE.BoxGeometry(0.2, 0.08, 0.25)),
				brassMat,
				0.18,
				-0.02,
				0,
		...
		group.add(
			createPart(
				getGeom("mg_stock", () => new THREE.BoxGeometry(0.48, 0.27, 0.2)),
				oliveMat,
				-0.55,
				0.01,
				0,
		...
		group.add(
			createPart(
				getGeom("mg_butt", () => new THREE.BoxGeometry(0.08, 0.3, 0.22)),
				gripMat,
				-0.82,
				0,
				0,
		...
		group.add(
			createPart(
				getGeom(
					"mg_pistol_grip",
					() => new THREE.BoxGeometry(0.14, 0.27, 0.14),
				),
				gripMat,
				-0.27,
				-0.23,
		...
		group.add(
			createPart(
				getGeom(
					"mg_carry_handle",
					() => new THREE.TorusGeometry(0.18, 0.025, 5, 10, Math.PI),
				),
				steelMat,
				0.02,
				0.31,
		...
		group.add(
			createPart(
				getGeom("mg_bipod_l", () => new THREE.BoxGeometry(0.045, 0.42, 0.045)),
				steelMat,
				0.6,
				-0.27,
				0.15,
		...
		group.add(
			createPart(
				getGeom("mg_bipod_r", () => new THREE.BoxGeometry(0.045, 0.42, 0.045)),
				steelMat,
				0.6,
				-0.27,
				-0.15,
		...
		group.add(
			createPart(
				getGeom("sg_recv", () => new THREE.BoxGeometry(0.42, 0.16, 0.14)),
				darkMat,
				-0.06,
				0.04,
				0,
		...
		group.add(
			createPart(
				getGeom(
					"sg_b1",
					() => new THREE.CylinderGeometry(0.045, 0.045, 0.88, 8),
				),
				steelMat,
				0.42,
				0.08,
		...
		group.add(
			createPart(
				getGeom("sg_pump", () => new THREE.BoxGeometry(0.28, 0.14, 0.16)),
				woodMat,
				0.14,
				-0.02,
				0,
		...
		for (let i = 0; i < 4; i++)
			group.add(
				createPart(
					getGeom(
						`sg_pring${i}`,
						() => new THREE.TorusGeometry(0.085, 0.015, 4, 12),
					),
					darkMat,
					0.06 + i * 0.05,
					-0.02,
		...
		group.add(
			createPart(
				getGeom("sg_stock", () => new THREE.BoxGeometry(0.48, 0.17, 0.17)),
				woodMat,
				-0.46,
				0.03,
				0,
		...
		group.add(
			createPart(
				getGeom("sg_comb", () => new THREE.BoxGeometry(0.35, 0.04, 0.18)),
				woodMat,
				-0.38,
				0.12,
				0,
		...
		group.add(
			createPart(
				getGeom("sg_bpad", () => new THREE.BoxGeometry(0.05, 0.18, 0.18)),
				gripMat,
				-0.72,
				0.03,
				0,
		...
		const tankMat = getMaterial("flamethrower_tank_red", () =>
			createWeaponMaterial(createPBRMetalTexture("#8f2c24")),
		);
		group.add(
			createPart(
				getGeom("ft_frame", () => new THREE.BoxGeometry(0.56, 0.2, 0.24)),
				darkMat,
				0.08,
				0.04,
				0,
		...
		group.add(
			createPart(
				getGeom(
					"ft_heat_shield",
					() => new THREE.CylinderGeometry(0.1, 0.1, 0.42, 10),
				),
				darkMat,
				0.68,
				0.08,
		...
		for (let i = 0; i < 4; i++)
			group.add(
				createPart(
					getGeom(
						`ft_vent_${i}`,
						() => new THREE.TorusGeometry(0.105, 0.012, 5, 10),
					),
					brassMat,
					0.53 + i * 0.1,
					0.08,
		...
		group.add(
			createPart(
				getGeom(
					"ft_igniter",
					() => new THREE.CylinderGeometry(0.085, 0.055, 0.18, 8),
				),
				accentMat,
				1.16,
				0.08,
		...
		group.add(
			createPart(
				getGeom("ft_pilot", () => new THREE.SphereGeometry(0.055, 8, 5)),
				neonMat,
				1.27,
				0.08,
				0,
		...
		group.add(
			createPart(
				getGeom("ft_grip", () => new THREE.BoxGeometry(0.15, 0.3, 0.14)),
				gripMat,
				-0.04,
				-0.22,
				0,
		...
		group.add(
			createPart(
				getGeom(
					"ft_tank_band_l",
					() => new THREE.TorusGeometry(0.155, 0.022, 6, 12),
				),
				brassMat,
				-0.48,
				-0.08,
		...
		const shellMat = getMaterial("laser_shell", () =>
			createWeaponMaterial(createPBRMetalTexture("#c8e3ed")),
		);
		group.add(
			createPart(
				getGeom("lz_spine", () => new THREE.BoxGeometry(0.8, 0.16, 0.18)),
				shellMat,
				0.08,
				0.08,
				0,
		...
		group.add(
			createPart(
				getGeom(
					"lz_core",
					() => new THREE.CylinderGeometry(0.055, 0.055, 0.62, 10),
				),
				neonMat,
				0.28,
				0.08,
		...
		group.add(
			createPart(
				getGeom(
					"lz_emitter",
					() => new THREE.CylinderGeometry(0.13, 0.09, 0.22, 10),
				),
				steelMat,
				0.84,
				0.08,
		...
		group.add(
			createPart(
				getGeom("lz_muzzle_orb", () => new THREE.SphereGeometry(0.08, 10, 6)),
				neonMat,
				1.08,
				0.08,
				0,
		...
		group.add(
			createPart(
				getGeom("lz_rear_cell", () => new THREE.SphereGeometry(0.19, 10, 6)),
				neonMat,
				-0.38,
				0.06,
				0,
		...
		group.add(
			createPart(
				getGeom(
					"lz_cell_cage",
					() => new THREE.TorusGeometry(0.21, 0.025, 6, 12),
				),
				darkMat,
				-0.38,
				0.06,
		...
		group.add(
			createPart(
				getGeom("lz_grip", () => new THREE.BoxGeometry(0.15, 0.3, 0.15)),
				gripMat,
				-0.12,
				-0.21,
				0,
		...
		group.add(
			createPart(
				getGeom(
					"lz_trigger_guard",
					() => new THREE.TorusGeometry(0.08, 0.014, 5, 10, Math.PI),
				),
				darkMat,
				0.02,
				-0.08,
...
function createArrowProjectileMesh() {
	...
	group.add(
		createPart(
			getGeom(
				"proj_arrow_shaft_g",
				() => new THREE.CylinderGeometry(0.042, 0.042, 1.9, 6),
			),
			shaftMat,
			0,
			0,
	...
	group.add(
		createPart(
			getGeom("proj_arrow_tip_g", () => new THREE.ConeGeometry(0.085, 0.28, 6)),
			tipMat,
			1.08,
			0,
			0,
	...
	for (let i = 0; i < 3; i++) {
		...
		group.add(
			createPart(
				getGeom(
					`proj_arrow_ts${i}`,
					() => new THREE.BoxGeometry(0.03, 0.15, 0.015),
				),
				tipMat,
				1.0 + Math.cos(a) * 0.06,
				Math.sin(a) * 0.06,
	...
	for (let i = 0; i < 3; i++) {
		...
		group.add(
			createPart(
				getGeom(
					`proj_arrow_f${i}`,
					() => new THREE.BoxGeometry(0.28, 0.1, 0.025),
				),
				fletchMat,
				-0.85,
				Math.cos(a) * 0.095,
	...
	group.add(
		createPart(
			getGeom("proj_arrow_nock_g", () => new THREE.BoxGeometry(0.06, 0.1, 0.1)),
			nockMat,
			-1.0,
			0,
			0,
...
function getThirdPersonGripForType(rawType) {
	...
	if (type === "rifle" || type === "machinegun")
		return { forward: 0.25, right: 0.12, up: -0.35 };
	if (type === "flamethrower" || type === "laser")
		return { forward: 0.24, right: 0.12, up: -0.35 };
...
function getWeaponModelTemplate(type) {
	...
	if (type === "knife") template.add(createKnifeModel());
		...
		template.add(createGunModel("flamethrower"));
...
export class Weapon {
	...
	attack(owner, target, audioSynth, directionOverride = null, options = null) {
		...
		if (!options?.ignoreCooldown && now - this.lastAttackTime < this.cooldown)
			return false;
			...
			const srcKey =
				ownerType === "Player"
					...
					: owner?.id !== undefined
						? `id:${owner.id}`
						: ownerType;
			if (this.type === "knife" || this.type === "fists")
				audioSynth.playHit?.(srcPos, srcKey);
	...
	rangedAttack(owner, target, directionOverride = null, options = {}) {
		...
		if (this.type === "shotgun") {
			...
			for (let i = 0; i < pelletCount; i++) {
				...
				pellet.lifetime = Math.max(
					0.2,
					this.getProfile().range / Math.max(1, pellet.speed),
				);
			...
			for (let i = 0; i < count; i++) {
				...
				flames.push(
					this.createProjectile(
						owner.position.clone(),
						direction.clone().add(spread).normalize(),
						"flame",
					),
				);
			}
	...
	createProjectile(startPos, direction, overrideType = null) {
		...
		if (type === "laser") {
			const m = getMaterial(
				"proj_laser_v2",
				() => new THREE.MeshBasicMaterial({ color: 0x7ff8ff }),
			);
			mesh = createPart(
				getGeom(
					"proj_laser_v2",
			...
			mesh = createPart(
				getGeom(
					"proj_bullet_v2",
					() => new THREE.CylinderGeometry(0.055, 0.055, 0.7, 8),
				),
				m,
				0,
				0,
			...
			mesh = createPart(
				getGeom("proj_generic", () => new THREE.ConeGeometry(0.1, 0.3, 8)),
				m,
			);
		}
	...
	animateAttack() {
		...
		if (!this._attackAnimationTimer) {
			if (!this._attackBasePosition)
				this._attackBasePosition = this.mesh.position.clone();
			else this._attackBasePosition.copy(this.mesh.position);
			if (!this._attackBaseRotation)
				this._attackBaseRotation = this.mesh.rotation.clone();
	...
	ensureFiniteTransform() {
		if (!this.mesh) return;
		if (
			![this.mesh.position.x, this.mesh.position.y, this.mesh.position.z].every(
				Number.isFinite,
			)
		)
			this.mesh.position.set(0, 0, 0);
		if (
			![this.mesh.rotation.x, this.mesh.rotation.y, this.mesh.rotation.z].every(
				Number.isFinite,
			)
		)
			this.mesh.rotation.set(0, 0, 0);
		if (
			![this.mesh.scale.x, this.mesh.scale.y, this.mesh.scale.z].every(
				Number.isFinite,
			)
		)
			this.mesh.scale.setScalar(getThirdPersonWorldScale(this.type));
