...
export class Player {
	...
	createMesh() {
		...
		const body = new THREE.Mesh(new THREE.BoxGeometry(0.8, 1.0, 0.6), bodyMat);
	...
	update(delta, audioSynth, lootManager, entityManager, controls) {
		...
		const moveVector = isFrozen
			? this._tmpZeroMove.set(0, 0, 0)
		...
		if (moveVector.length() > 0) {
			...
			if (
				!isFrozen &&
				heldWeapon?.type === "bow" &&
				this.input.isKeyPressed("MouseLeft")
			) {
			...
			if (
				...
				currentTime - this.lastFootstepTime > 0.5 &&
				audioSynth
			) {
				audioSynth.playFootstep(this.footstepVolume);
				const surfaceType = this.mapRef?.getSlowZoneTypeAt?.(
					this.position.x,
					this.position.z,
				);
				if (surfaceType === "glass") {
					audioSynth.playGlassStep?.(
						this.position,
						`player-${Math.floor(currentTime * 10)}`,
					);
				}
		...
		for (let displaySlot = 1; displaySlot <= 10; displaySlot++) {
			...
			if (
				(this.input.isKeyPressed(`Digit${key}`) ||
					this.input.isKeyPressed(`Numpad${key}`)) &&
				this.inventory.hasItem(slotIndex)
			) {
		...
		const autoTarget =
			this.autoFire && activeWeapon.type !== "bow"
				? this.getAutoFireTarget(entityManager)
				: null;
		const isRangedWeapon = [
			...
			"machinegun",
			"bazooka",
		].includes(activeWeapon.type);
		const fireHeld = this.input.isKeyPressed("MouseLeft");
		const fireRequested =
			fireHeld || (!!autoTarget && !isFrozen && isRangedWeapon);
		if (activeWeapon.type === "bow") {
			...
			const shouldReleaseBow =
				!fireHeld && this.wasFireHeld && this.bowCharge >= this.bowMinCharge;
			if (!isFrozen && shouldReleaseBow && this.attackCooldown <= 0) {
				...
				const chargeRatio = Math.max(
					0.35,
					Math.min(1, this.bowCharge / this.bowChargeMax),
				);
				...
				if (result && result.projectiles) {
					...
					if (result.projectile.velocity) {
						result.projectile.velocity
							.copy(direction)
							.multiplyScalar(result.projectile.speed);
					}
				...
				this.attackCooldown =
					Math.max(0.35, activeWeapon.cooldown * (0.95 - chargeRatio * 0.3)) *
			...
			if (
				activeWeapon.type === "laser" ||
				activeWeapon.type === "shotgun" ||
				activeWeapon.type === "flamethrower" ||
				activeWeapon.type === "pistol" ||
				activeWeapon.type === "rifle" ||
				activeWeapon.type === "machinegun" ||
				activeWeapon.type === "bazooka"
			) {
				const direction = this._tmpFireDir;
				if (autoTarget) {
					direction
						.subVectors(autoTarget.position, controls.camera.position)
						.normalize();
				} else {
					controls.getWorldDirection(direction);
				}
				...
				this.viewKick =
					(recoilByType[activeWeapon.type] || 0.2) * this.recoilScale;
				this.weaponActionTime = this.weaponActionDuration;
				this.weaponActionType = activeWeapon.type;
			} else {
				const target = entityManager.getNearestEnemy(
					this.position,
					activeWeapon.range,
				);
				if (target) {
					const result = activeWeapon.attack(this, target, audioSynth, null, {
						ignoreCooldown: true,
					});
		...
		if (!isFrozen && this.input.isKeyPressed("KeyE")) {
			...
			if (poi) {
				...
				if (type === "weapon") {
					...
					const index =
						Math.abs(Math.round(poi.position.x * 13 + poi.position.z * 7)) %
						weapons.length;
					...
					this.inventory
						.getItems()
						.some((item) => item?.ammo !== null && item?.ammo < item?.maxAmmo)
				) {
	...
	animateViewModel(isFirstPerson, delta) {
		...
		if (this.viewWeapon && this.viewWeapon.visible) {
			...
			this.currentWeapon?.anim?.update(
				delta,
				isShooting,
				speed > 0.3,
				mDx,
			...
			this.currentWeapon?.anim?.applyToMesh(
				this.viewWeapon,
				this.viewWeaponType,
			);
			...
			if (
				(this.viewWeaponType === "bow" ||
					this.viewWeaponType === "laser" ||
					this.viewWeaponType === "shotgun" ||
					this.viewWeaponType === "pistol" ||
					this.viewWeaponType === "rifle" ||
					this.viewWeaponType === "flamethrower" ||
					this.viewWeaponType === "machinegun" ||
					this.viewWeaponType === "bazooka") &&
				this.viewKick > 0
			) {
			...
			if (this.weaponActionTime > 0 && this.weaponActionType) {
				const t = THREE.MathUtils.clamp(
					1 - this.weaponActionTime / this.weaponActionDuration,
					0,
					1,
				);
				...
				if (this.weaponActionType === "bow") {
					...
					this.weaponActionType === "laser" ||
					this.weaponActionType === "shotgun" ||
					this.weaponActionType === "pistol" ||
					this.weaponActionType === "rifle" ||
					this.weaponActionType === "flamethrower" ||
					this.weaponActionType === "machinegun"
				) {
					this.viewWeapon.position.z -= ease * 0.1;
					this.viewWeapon.rotation.x -= ease * 0.35;
				} else if (this.weaponActionType === "bazooka") {
					this.viewWeapon.position.z -= ease * 0.25;
					this.viewWeapon.rotation.x -= ease * 0.7;
				}
			}
		}
	}
	...
	selectSlot(slot) {
		...
		const isFirstPerson =
			this._stableFirstPerson === true || this.input?.isMobile === true;
		this.updateFirstPersonArmsVisibility(isFirstPerson);
		if (this.hudRef) {
			this.hudRef.updateAmmo(this.currentWeapon);
		}
	...
	pickupLoot(loot) {
		...
		if (loot.type === "weapon") {
			...
			if (result.added) {
				...
				if (result.slot >= 0) {
					feedParts.push(
						`Пополнение: ${this.getWeaponDisplayName(loot.weaponType)}`,
					);
				}
				weapon.dispose();
			}
			...
			if (amount > 0) {
				const candidates = this.inventory
					.getItems()
					.filter((w) => w && w.ammo !== null);
				const target =
					this.currentWeapon && this.currentWeapon.ammo !== null
						? this.currentWeapon
						: candidates[0];
				if (target) {
					const before = target.ammo ?? 0;
					target.ammo = Math.min(
						...
						(target.ammo ?? 0) + amount,
					);
					const gained = Math.max(0, (target.ammo ?? 0) - before);
					if (gained > 0) {
						feedParts.push(
							`${this.getWeaponDisplayName(target.type)}: +${gained} патр.`,
						);
					}
				}
			}
			...
			this.armor = Math.min(
				this.maxArmor,
				this.armor + Math.round(healAmount * 0.12),
			);
		...
		if (loot.bonusAmmo) {
			const gained = this.addAmmoToWeaponType(
				loot.bonusAmmo.weaponType,
				loot.bonusAmmo.amount,
			);
			if (gained > 0) {
				feedParts.push(
					`${this.getWeaponDisplayName(loot.bonusAmmo.weaponType)}: +${gained} патр.`,
				);
			}
		}
		this.stats.loot += 1;
	...
	takeDamage(
		...
		if (this.health <= 0) {
			...
			this.mesh.position.y =
				this.position.y - (this.physics.height - 0.15) - 0.8;
	...
	applyWeaponPerk(weapon) {
		...
		if (this.perk === "moreAmmo") {
			...
			if (weapon.maxDurability !== null) {
				weapon.maxDurability = Math.round(
					weapon.maxDurability * this.perkAmmoBonus,
				);
	...
	updateHealthRegen(delta) {
		...
		this.health = Math.min(
			this.maxHealth,
			this.health + regenPerSecond * delta,
		);
	}
	...
	animateViewModelWeapon(weaponType) {
		...
		try {
			...
			const offset = (this.getViewWeaponOffset &&
				this.getViewWeaponOffset(weaponType)) || {
				scale: 0.8,
			...
			this.viewWeapon.userData.baseRotation = new THREE.Euler(
				this.viewWeapon.rotation.x,
				this.viewWeapon.rotation.y,
				this.viewWeapon.rotation.z,
			);
		} catch (e) {
	...
	getAutoFireTarget(entityManager) {
		...
		for (const entity of entityManager.getEntities()) {
			...
			const toTarget = this._tmpAutoToTarget.subVectors(
				entity.position,
				origin,
			);
	...
	attack(target, entityManager) {
		...
		if (
			...
			// Custom recoil per weapon type
			weapon.anim.recoilAmount =
				weapon.type === "bazooka" ? 3 : weapon.type === "shotgun" ? 1.8 : 1;
			const direction = this._tmpAttackDirection
				.subVectors(target.position, this.position)
				.normalize();

			const projectileData = weapon.attack(
				this,
				null,
				this.audioSynthRef,
				direction,
